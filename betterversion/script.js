export function automationScript(promptText, cardName) {
  ...
  // --- Throttled Report Function ---
  let throttleTimeout = null;
  const THROTTLE_DELAY = 5000; // Update at most once every 5 seconds
  let lastReportedText = '';

  const report = (txt, done, err = false) => {
    const finalText = err ? `[AUTOMATION ERROR] ${txt}` : txt;

    // If this is the final message, we must send it immediately, bypassing any throttling.
    if (done) {
      if (throttleTimeout) clearTimeout(throttleTimeout); // Cancel any pending throttle reset
      throttleTimeout = null;
      chrome.runtime.sendMessage({ action: "updateDashboard", responseText: finalText, isComplete: true, timestamp: new Date().toLocaleString() });
      return;
    }

    // If we are currently in a throttled "cooldown" period, do nothing.
    if (throttleTimeout) return;

    // If the text hasn't actually changed, don't send a redundant update.
    if (finalText === lastReportedText) return;

    // Send the update now
    lastReportedText = finalText;
    chrome.runtime.sendMessage({ action: "updateDashboard", responseText: finalText, isComplete: false, timestamp: new Date().toLocaleString() });

    // Start the cooldown. No more messages will be sent until it finishes.
    throttleTimeout = setTimeout(() => { throttleTimeout = null; }, THROTTLE_DELAY);
  };
  const badge = (status) => {
